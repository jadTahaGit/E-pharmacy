<?php
    ...
    $nbr = getItems("medcine.id","type,medcine","type.id=medcine.type_id and type.id='$categ' order by medcine.id limit $limit offset $offset_check");
    $nbr_rows = mysqli_num_rows($nbr);
    $res = getItems("medcine.id as med_id,medcine.name as med_name,medcine.description as med_description,price,picture_front","type,medcine","type.id=medcine.type_id and type.id='$categ' order by $sort_by limit $limit offset $offset");
    $result = "";
