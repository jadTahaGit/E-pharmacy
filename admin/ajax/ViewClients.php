<?php
    ...
    {
        $res = getItems("user.id,firstname,lastname,email,telephone,city,address,sex","user,client","user.id=client.user_id");
        while($row = mysqli_fetch_row($res))
        {
            echo '
                <tr id="row_'.$row[0].'">
                    <td style="width: 12.5%;">'.$row[1].'</td>
                    <td style="width: 12.5%;">'.$row[2].'</td>
                    <td style="width: 15%;"><button class="btn btn-link" onClick="reply(`'.$row[3].'`)">'.$row[3].'</button></td>
