<?php
    ...
    {
        $res = getItems("p.id,concat(u.firstname,' ',u.lastname) as name,u.email,c.telephone,p.prescription_pic,p.date_time","user u,client c,prescription p","u.id=c.user_id and u.id=p.user_id");
        while($row = mysqli_fetch_row($res))
        {
            echo '
                <tr id="row_'.$row[0].'">
                    ...
                    <td style="width: 10%;" align="center"><button class="btn btn-primary" name="open" id="open_'.$row[0].'" onClick="openn(`'.$row[4].'`)">&nbsp;&nbsp;&nbsp;
