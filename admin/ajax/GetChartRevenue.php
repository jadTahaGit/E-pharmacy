<?php
    ...
    if(isset($_COOKIE['idpharmacyadmin']) && isset($_COOKIE['emailpharmacyadmin']) && isset($_COOKIE['passwordpharmacyadmin']))
    {
        ...
        for($i = 29, $j = 0; $i > -1; $i--, $j++)
        {
            ...
            $thisd = $arr_dates[$j];
            $amount = getItems("amount","revenue","date_time like '$thisd%'");
            $thisdateamount = 0;
            while($c_amount = mysqli_fetch_row($amount))
            {
                if($c_amount[0] == null)
                    $c_amount[0] = "0";
                $total_amount += $c_amount[0];
                $thisdateamount += $c_amount[0];
            }
                $arr_amount[$j] = $thisdateamount;
        }
